...
** hexadecimal_maj_specifier_2
** File description:
** hexadecimal maj specifier 2
*/
...
void hexadecimal_maj_specifier_ll(flags_manager_t *manager)
{
    ...
    put_unsigned_long_long_maj_in_buffer(manager, nb, len, BASE_16);
...
void hexadecimal_maj_specifier_j(flags_manager_t *manager)
{
    ...
    put_uintmax_t_maj_in_buffer(manager, nb, len, BASE_16);
    if (manager->is_pad_left == 0)
        put_padding(manager, len + is_alt, ' ');
}
...
void hexadecimal_maj_specifier_z(flags_manager_t *manager)
{
    ...
    put_size_t_maj_in_buffer(manager, nb, len, BASE_16);
    if (manager->is_pad_left == 0)
        put_padding(manager, len + is_alt, ' ');
}
